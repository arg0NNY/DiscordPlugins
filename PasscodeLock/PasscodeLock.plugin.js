  ...
  componentDidMount () {
    ...
    this.disableKeys = e => {
      ...
      if (e.ctrlKey && e.shiftKey && (e.key === 'I' || e.key === 'C')) {
        ...
        e.stopPropagation()
        return false
      } // Prevent all sorts of shortcuts like bold, italic, underline, strikethrough, ...
      else if (e.key === 'Enter') {
...
module.exports = class PasscodeLock {
  static Types = {
    FOUR_DIGIT: '4-digit',
    SIX_DIGIT: '6-digit',
  ...
  lock ({ button, type, onSuccess } = {}) {
    ...
    if (this.settings.hash === -1 && type !== PasscodeLocker.Types.EDITOR) return UI.showToast(Locale.current.FIRST_SETUP_MESSAGE, { type: 'error' })
  ...
  patchWindowInfo () {
    Patcher.instead(WindowStore, 'isFocused', (self, props, original) => {
...
`)
  ...
  patchSettingsButton () {
    ...
    const callback = e => {
      ...
      if ((node = e?.querySelector(`#${this.getName()}-card .bd-controls > .bd-button:first-child`))) {
        ...
        patchedNode.onclick = () => {
          if (!Plugins.isEnabled(this.getName())) return
  ...
  async updateCode (code) {
    ...
    UI.showToast(Locale.current.PASSCODE_UPDATED_MESSAGE, { type: 'success' })
  }
  ...
  getSettingsPanel () {
    if (!this.KeybindRecorder) {
      this.KeybindRecorder = Webpack.getModule(m => m.prototype?.handleComboChange)
    }
    ...
    const settingsNode = UI.buildSettingsPanel({
      onChange: () => {
      ...
      settings: [
        ...
        },
        {
          type: 'radio',
          id: 'codeType',
          name: Locale.current.CODE_TYPE_SETTING,
          value: this.settings.codeType,
          options: [
            {
          ...
          onChange: e => {
            ...
            UI.showToast(Locale.current.PASSCODE_RESET_DEFAULT_MESSAGE, { type: 'warning' })
        ...
        },
        {
          ...
          children: React.createElement(Slider, {
            ...
            stickToMarkers: true,
            onMarkerRender: e => e === -800 ? 'OFF' : e < 60 * 60 ? `${e / 60}${e < 10 * 60 ? '' : 'm'}` : `${e / 60 / 60}h`,
            initialValue: this.settings.autolock === false ? -800 : this.settings.autolock,
            onValueChange: e => {
        ...
        },
        {
          type: 'custom',
          id: 'keybind',
          name: Locale.current.LOCK_KEYBIND_SETTING,
          inline: false,
          children: React.createElement(this.KeybindRecorder, {
        ...
        },
        {
          type: 'switch',
          id: 'lockOnStartup',
          name: Locale.current.ALWAYS_LOCK_SETTING,
          note: Locale.current.ALWAYS_LOCK_DESC,
          value: this.settings.lockOnStartup,
          onChange: e => {
        ...
        },
        {
          type: 'switch',
          id: 'highlightButtons',
          name: Locale.current.HIGHLIGHT_TYPING_SETTING,
          note: Locale.current.HIGHLIGHT_TYPING_DESC,
          value: this.settings.highlightButtons,
          onChange: e => {
        ...
        },
        {
          ...
          name: Locale.current.NOTIFICATIONS_SETTING,
          value: this.settings.hideNotifications,
          options: [
            {
  ...
  constructor () {
    ...
    if (this.settings.code || [10000, 1000].includes(this.settings.iterations)) {
      ...
      UI.showToast(Locale.current.PASSCODE_RESET_SECURITY_UPDATE_MESSAGE, { type: 'warning' })
    }
